...
int main() {
    string numero = "73167176531330624919225119674426574742355349194934"
                    ...
                    "12540698747158523863050715693290963295227443843557"
    ...
    long long mayorProducto = 0;

    for (int i = 0; i <= numero.length() - digitosAdyacentes; i++) {
        long long producto = 1;

        // Calcula el producto de la secuencia actual
        for (int j = 0; j < digitosAdyacentes; j++) {
            producto *= numero[i + j] - '0';
        }

        // Actualiza el mayor producto si es necesario
        if (producto > mayorProducto) {
            mayorProducto = producto;
        }
    }

    cout << "El mayor producto de " << digitosAdyacentes << " dígitos adyacentes es: " << mayorProducto << endl;
